	...
	public class GlobalMultiLine : GlobalItem
	{
		...
		public override bool Shoot(Item item, Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
		{
			if(item.fishingPole != 0)
			{
                ...
                if (item.type == ModContent.ItemType<Items.Poles.TwinPole>()) //TODO: was Mod.Find<ModItem>("TwinPole").Type
                {
					playerLineCount += 1 + (int)System.Math.Log(playerLineCount, 2); //TODO: why use logarithm? And why use "+= 1"?
				}
				...
				for(int i = 1; i < playerLineCount; i++)
				{
					Projectile.NewProjectile(
                        source,
                        position.X + (player.direction * (float)i / (float)System.Math.Log( (float)playerLineCount)),
						position.Y,
                        velocity.X + (player.direction * (float)i / (float)System.Math.Log( (float)playerLineCount )),
                        velocity.Y,
						type,
						damage,
                        knockback,
