	...
	public abstract class Bobber : ModProjectile
	{
		...
		public override bool PreDrawExtras()
		{
			...
			if(player.inventory[player.selectedItem].holdStyle > 0)
			{
				...
				if (player.direction < 0)   pos.X -= 13.0f;
				if (player.gravDir == -1f)   pos.Y -= 12.0f;
				pos = player.RotatedRelativePoint(pos + new Vector2(8.0f), true) - new Vector2(8.0f);
		...
		public override bool PreDraw(ref Color lightColor)
		{
			if(light.X > 0 || light.Y > 0 || light.Z > 0)
			{
				...
				else Lighting.AddLight(Projectile.Center, light);
			}
			return true;
		}
	}
